...
function get_initial(x0, multiple_initial_solutions, N)
    if multiple_initial_solutions
        return [[x0]; [rand(2) * 10 for _ in 1:N-1]]
    else
        return x0
    end
end

@testset "Multiple initial solutions: $multiple_initial_solutions" for multiple_initial_solutions in (true, false)
    ...
    @testset "Simple constraints" begin
        ...
        @test abs(r.minimum - 100sqrt(8/27)) < 1e-2
        @test norm(r.minimizer - [1/3, 8/27]) < 1e-2
    end
